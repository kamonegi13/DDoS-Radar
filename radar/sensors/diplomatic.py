...
def _article_hash(source_name: str, title: str, theater: str) -> str:
    raw = f"dipl-{source_name}-{title[:60]}-{theater}"
...
class DiplomaticSensor(BaseSensor):
    ...
    def fetch(self, context: dict) -> dict:
        ...
        for source_name, meta in _DIPLOMATIC_SOURCES.items():
            ...
            # Build per-theater analysis batches
            for theater in theaters:
                # Check if any article is already processed
                new_articles = []
                for art in articles:
                    key = _article_hash(source_name, art["title"], theater)
                    if key not in _processed:
                        new_articles.append((key, art))
                if not new_articles:
                    continue

                # Build LLM prompt from batch of new articles
                ...
                system_prompt = (
                    ...
                    f"relevant to the {theater} theater. "
                ...
                user_prompt = (
                    ...
                    "If statements have no relevance to the theater, return confidence<0.40."
                )

                result = llm_analyze_json(user_prompt, system=system_prompt, max_tokens=256)

                # Mark all as processed regardless of LLM result
                for key, _ in new_articles:
                    _processed.add(key)
                ...
                if not result["ok"]:
                    log.debug(f"[Diplomatic] LLM parse failed {source_name}/{theater}: {result.get('error')}")
                    continue
                ...
                if not data.get("escalation_signal", False) or confidence < 0.55:
                    log.debug(f"[Diplomatic] No signal {source_name}/{theater} conf={confidence:.2f}")
                    continue

                urgency = data.get("urgency", "low")
                score_delta = {"critical": 3.0, "high": 2.0, "medium": 1.5, "low": 1.0}.get(urgency, 1.0)

                # Use first new article's link as raw_url
