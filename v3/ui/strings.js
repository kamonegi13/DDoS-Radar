...
(function (root, factory) {
    ...
    const STRINGS = {
        ...
        'ui.board.since.worsened': '前回確認（{ago}前）から脅威レベルが悪化。TL {previous} → {current}（severity {delta}）。',
        'ui.board.since.improved': '前回確認（{ago}前）から脅威レベルが改善。TL {previous} → {current}（severity {delta}）。',
        'ui.board.since.unchanged': '前回確認（{ago}前）から脅威レベルに変化なし（TL {previous}）。',
