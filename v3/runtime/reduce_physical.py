"""The physical-domain folds. Half of the reductions, by module.
...
**The order inside each fold is the finding, not a detail.** Production
sums a country's ISR zones BEFORE thresholding, pools three RIPE Atlas
measurements BEFORE taking a p95, and ORs the two space-weather
suppressors. Thresholding before summing does not merely round
differently; it removes a detection, which under NP1 is the worst
available outcome.

Design sheet §3-5 H-1(b)/(c) and §7-2 #11/#12/#13/#23/#28/#29/#31.
"""
...
from v3.adapters.types import (PHYSICAL, STATUS_FIRED, STATUS_NO_DATA,
                               STATUS_OBSERVED, STATUS_OK, STATUS_SUPPRESSED,
                               ObservationDraft)
from v3.runtime import verdicts
from v3.runtime.reduce_common import (ATLAS_BLACKOUT, ATLAS_DROP,
                                      CHECKHOST_URL_OK_RATE, PENDING_PREFIX,
                                      SPACE_WEATHER_KP_THRESHOLD,
                                      SPACE_WEATHER_XRAY_CLASS, _by_country,
                                      _coverage, _first_url, _measured,
                                      _min_confidence, _mmsi_last_seen)
