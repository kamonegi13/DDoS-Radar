...
> **2026-08-09 訂正（ADR-V3-011）— 本表は上書きされた。正は [P2-migration-plan.md](P2-migration-plan.md) §5**。
> 上表は残す（当時の主張が読めることが本プログラムの規律）が、**cutover 判定に用いてはならない**。
> 主な差分: (a) 条件数 14 → **17**、(b) CUT-01 の要素単位一致率は**帰属の完全性**（未登録の不一致 0 件）
> へ置換、(c) **CUT-07 の述語を差し替え**（非退化に加えて「P2 §5-0 の VOID 参照点に依存しない」を要求。
> 閾値 4 の導出根拠が本仕様に記録されていない件も P2 §5-A に記録）、(d) CUT-10 は P2 C-07 に統合、
> (e) CUT-14 は P2 C-16（連続 30 日の無故障走行）に包含、(f) 新設 3 件（P2 C-15 / C-16 / C-17）。
> **override 禁止集合も改訂**: CUT-02 / CUT-08 / CUT-11 に加え、**P2 C-03 の insensitive 不一致**と
> **P2 C-17（cutover 阻害欠陥ゼロ）**が禁止側に入る（S5-VERIF-043 の一覧より P2 §5 が優越）。
...
> **2026-08-09 裁定（ADR-V3-011）— A1 / A2 に決着がついた**:
> - **A1**: 一致率の下限（CUT-01 0.98 / CUT-04 の型別下限）は**合否の閾値から降ろした**。P2 C-03 は
>   「未登録の不一致 0 件」を条件とし、一致率は**開示必須・閾値なし**の指標として残る。
>   「実測に基づかない設定値」という指摘は正しく、解は**数字を実測で決めること**ではなく
>   **数字を合否に使わないこと**だった（一致率 0.96 で未登録差分が残る状態が旧条件では通った）。
>   **CUT-04 の型別下限は P2 C-03 の帰属条件に吸収**。
> - **A2**: **CUT-07 は廃止せず、述語を差し替えて P2 C-15 として存続**。指摘された 2 択
>   （ラベル生成器で attack_mode の TN/FN を出す / attack_mode を対象外と明示）に**第 3 の欠落**が
>   あった — 現行 3 cell の recall は `ripe_bgp`（H-05、全期間死亡）・`gps_jamming`（F-08）・
>   `telegram_mirror`（F-06）が**死んだまま**算出されている。したがって有効 cell の述語には
>   「P2 §5-0 の VOID 参照点に依存しない」が要る。**数は増えるどころか減りうる**。
>   負例ラベル収集は暦時間の律速項目として P2 §7 に載せた。
