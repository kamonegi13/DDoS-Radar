...
| 137 | `bg_observer_rss` — **9 フィードをまたぐ畳み込みが未実装で、L1 の UNIQUE 制約に毎ティック衝突していた**（**G-19 の 1 件目、2026-08-09**） | `normalize` は 1 フィード本文の中で国ごとに MAX を取るところまでは実装済みで、**「9 本をまたぐ畳み込みは WP-4.1 に属する」と自分の docstring に書いたまま置かれていた**（`bg_observer_rss.py:30-37`）。アダプタは `RequestSpec` を 9 本宣言し、`normalize` は §2-2 barrier 1 により 1 ペイロードしか見ないので、**2 本の通信社が同じ国を報じた瞬間に `(bg_observer, IR)` が 2 行**になり、`reduce._require_writable` が `DomainError` を投げてティック全体が採点前に落ちる。実測: 影コンテナで 6 ティック連続同一失敗、`tl_observation` は 0 行のまま。**採択した畳み込みは MAX**であり、これは本番が同じ族に対して実際に計算しているものである — 観測側は (country, item) ごとに Signal を投げるだけで畳まず（`background_observer.py:369-401`）、採点側の `dedup_by_source_country_max`（`radar/scoring.py:1424-1432`）が `(signal_source, contributing_country)` ごとに `final_contribution` 最大の 1 件を残す。この族では `final = raw_score × 1.0 × participant_weight` である（drain が `country_weights={c: 1.0}` を与える、`radar/routes/core.py:2103-2113`）ため、**`raw_score` での順位付けと `final_contribution` での順位付けは同じ行を選ぶ** | **中立**（本番の畳み込みと同値。SUM を採れば 9 本が 1 つの事件を報じたときスコアが 9 倍になり、本番が「重複は dedup される前提で投げている」と明記しているもの〔`background_observer.py:276-279`〕に真っ向から反する — 唯一の誤答であり、採らなかった） | **登録**（WP-D、2026-08-09）。負けたフィードは捨てない: `feeds_matched` / `items_seen_total` / `folded[]` が各社の点数と表示行を残す — 「1 社だけがイランに触れた」と「7 社が触れた」は corroboration についての別の事実であり、勝者の数値しか畳み込みを生き残らないため（NP6）。**解消条件**: 無し（本番と同値の恒久畳み込み）。証跡: `tests/test_runtime_reduce.py::TestBgObserverFoldsAcrossTheNineFeeds`（MAX であること・SUM でないこと・国が独立に畳まれること・敗者が開示されること） |
| 138 | **候補記事アダプタ 4 基 — 記事ごとの行を、本番が持つ 1 行に畳んだ**（`apt_intel` / `diplomatic` / `military_exercise` / `hacktivist_news`、**G-19 の 2 件目、2026-08-09**） | **4 基は同一構造である**: それぞれ 5〜11 本のフィードを宣言し、`normalize` は 1 ペイロードしか見ず（§2-2 barrier 1）、選抜記事 1 本につき countryless の `OBSERVED` draft 1 つと、死んだフィードにつき `NO_DATA` draft 1 つを出す。全部が各アダプタ唯一の `SIGNAL_SOURCE` と `country=''` を共有するので、**2 本のフィードが同じサイクルで記事を出した瞬間に L1 の `UNIQUE (tick_id, sensor, signal_source, country)` に衝突し、採点前にティックが落ちる**。**畳み込みは 4 つではなく 1 つ**（`reduce_common.fold_candidate_articles`）— 設計シート自身の A-02 の発見は「1 つのプロンプトの 8 つの複製が乖離した」ことであり、1 つの畳み込みの 4 複製はそれを防ぐために作った層で同じ過ちを犯すことになる。移植は選抜記事 1 本につき 1 draft を出し、全部が `signal_source='apt_intel'` / `country=''` を共有していたため、L1 の `UNIQUE (tick_id, sensor, signal_source, country)` に毎ティック衝突していた（#137 と同じ落ち方）。**形を選ぶ前に本番を測った結果、本番は 4 基とも記事ごとの根拠行を採点面に出していない**: 4 つのセンサーはすべて `log_fetch(any_feed_ok, ms, 0, submitted)` と `set_cache({<名前>: {'submitted': N}})` で終わり（`radar/sensors/apt_intel.py:605-608`、`diplomatic.py:568-570`、`military_exercise.py:438-440`、`hacktivist_news_sensor.py:466-468`）、**整数 1 個を持つキャッシュ 1 件**しか残さない。そのキャッシュの唯一の読者は `convergence_tracker.py:132-139` の `cache.get(<名前>, {}).get('submitted', 0) > 0`、すなわち**真偽値**である。`Signal` は 1 つも作られないので、**本番の 4 族から各シナリオスコアへの寄与は厳密に 0**。記事が採点に届く経路は別にあり、`intel_queue.submit(item)` を通って LLM が国を付けた `llm_intel` になる。v3 もその経路を持つ（`v3/adapters/llm/extraction.py` が唯一の抽出面）が、**`v3/runtime/` からこれを呼ぶ箇所が 1 つも無い** — `apt_intel` は `IntelProfile` すら持たず、他 3 基は宣言だけがあって起動されない。つまり 4 基とも「配線されていない段を待つ候補」という同じ状態にある。したがって採択形は **countryless の OBSERVED 1 行・`raw_score=0.0`**、記事全件を `flags['articles']` に載せる。`cf_spike_target`（#118 の着地）と `ooni_heavy`（#131）と同型の「採点面を持たない作業値を読者に届ける」形であり、**名前を発明しない点だけが違う** — 記事は本番が既に持っている行に収まる | **中立**（両系とも本族からシナリオスコアへの寄与は 0。畳み込みが変えるのは同じ記事を運ぶ L1 の行数だけ） | **登録**（WP-D、2026-08-09）。**記事ごとに `signal_source` を発明する案は退けた**: 1 ティックあたり約 35 個、保持期間で数万個の新しい値を、**L1 の dedup 兼結合キーである列**に入れることになる。#118 と #131 はそれぞれ**固定の 1 名前**を**名前の付いた実在の読者 1 人**のために足したものであり、まだ存在しない読者のための無限族はその先例ではなく先例の不在である。**記事は畳んでも失われない**: `dedup_key` は `radar/sensors/apt_intel.py:173-175` の鍵そのものを記事ごとに保持しており（B-05 が台帳へ移した当のもの）、抽出段が着地したときに個別に addressable である。死んだフィードも畳み込みを生き延びる — `feeds_dead[]` が parse outcome を残し、**候補が 1 件も出なかったサイクルは OBSERVED ではなく NO_DATA** になる（DP9 の修理: 「CISA がエラーページを配っている」と「CISA が今週何も出していない」が同じに印字されてはならない）。**解消条件**: WP-2.7 が `apt_intel` に `IntelProfile` を与えたとき — 記事は本番の実際の採点経路どおり `llm_intel` 観測になり、`flags['articles']` は終着点ではなく抽出段の入力になる。**残余の非対称を明記する**: 本番は `_processed` によりサイクルをまたいで再投入を抑止するが、v3 は 168h 窓にある記事を毎ティック再掲する。読者が居ないので現時点では中立。**列挙そのものが不健全だったことを記録する（本作業中に影配備が証明）**: 当初の診断は「1 サイクル走らせて違反を収集する」方法で `apt_intel` と `bg_observer_rss` の 2 件を『全件』として報告したが、**`diplomatic` は 11 本中 2 本が同じサイクルで記事を出したときにだけ衝突する**ため、静かなサイクルは「衝突しないアダプタ」という誤った健全証明を出す。修理後の影配備は 1 分以内に `diplomatic` で落ちた。**したがって危険集合は標本ではなく宣言から導く**: `normalize` は 1 ペイロードしか見ないので、**リクエストを 2 本以上宣言するアダプタはすべて危険**である。`tests/test_runtime_reduce.py::TestNoAdapterCanGrowThisDefectAgain` が全アダプタ宣言に対してこれを主張し、免除は `gps_jamming` 1 件のみ（manifest ペイロードが `()` を返すので draft を作るのは 1 本だけ、かつ国ごとに一意）。証跡: `tests/test_runtime_reduce.py::TestAptIntelFoldsToTheOneRowProductionHas` / `::TestNoAdapterCanGrowThisDefectAgain` |
| 139 | 取得カーネル全体 — **1 ティックあたりの取得予算（v3 独自機構）**（**G-19 の 3 件目、2026-08-09**） | `is_due` は「一度も走っていない」アダプタに True を返す（正しい。冷配備が初観測まで 1 cadence 待つ理由は無い）。その帰結として **`fetch_schedule` が空のとき全アダプタが同一ティックで due になる**。実測（配備実機の geography、4 シナリオ / 27 参加国）: 初回計画は **展開後 772 step・最大 935 リクエスト**。影コンテナでの実走 2 回はいずれも 25 分後もまだ HTTP ソケットの `ppoll` の中にいて、採点段に到達せず `tl_observation` は 0 行だった。**そして掃引が終わらないので `record_fetch` が commit されず、`fetch_schedule` が進まず、次ティックが同じ 772 を計画する** — 欠陥 A と合わせてこれが livelock の正体である。本番にこの形は無い: 本番はセンサーごとに独立したスレッドとタイマーを持つので「全部が同時に due」という状態自体が存在しない。したがって予算は**移植ではなく v3 独自の機構**であり、登録対象である。採択: `apply_budget`（純粋関数、`v3/fetch/runner.py`）が計画を **`FETCH_BUDGET_REQUESTS = 120` step** で切り、あふれた分を `fetch_budget_deferred` 理由で `skipped` へ移す。**120 は選んだのではなく導いた**: `ops_health.LOOP_MIN_OVERDUE_SEC` = 300s が本プログラム自身の健全性面が「遅い」と判定する境界であり、影台帳の実測は掃引全体で約 2.5 s/request（311 リクエストで 25 分超。健全時のみの p95 は 0.6s なので、平均を作っているのは timeout / retry 経路 — 死んだホスト 1 本で 3 試行 ×〔5s connect + 20s read〕+ backoff = 78s）。120 × 2.5s = 300s | **insensitive**（ある観測が最大数ティック遅れて台帳に入る）**だが、代替は「ティックが一度も完了しない」であり、そちらは恒久的な結論不可 = NP5+8 の設計失敗**。冷掃引は 7 ティック（実測）で完了し、`first_tick_grace`（1800s）に十分収まる | **登録**（WP-D、2026-08-09）。設計上の要点 3 つを明記する。**(a) 単位はアダプタであり、原子的である。** 自分 1 つで予算を超えるアダプタ（`ct_log` = 162 step）も丸ごと走る。半分だけ取得することが唯一の禁じ手で、`reduce` はアダプタのペイロードをまたいで畳むので、162 ドメイン中 80 だけ取得した `ct_log` は**完全なものと見分けのつかない verdict を畳む** — G-19 の修理が G-17 を製造する形。**(b) 順序は「最後に喋ってから最も長い順」**（`last_run_at` 昇順、未実行が先）。宣言順でも偶然 starvation-free ではある（全 cadence が 300s 以上、ループは 60s なので走ったアダプタは 5 ティック due 集合を離れる）が、**偶然でしかなく**、30s cadence のアダプタが 1 つ宣言された瞬間に予算を独占する。実行した瞬間に最新になって最後尾へ回る順序は構成的に starvation-free である。**(c) 抑制の生産者（`space_weather` / `openweather`）は予算免除。** 1 つ延期すると**走った側の読み方が変わる**（生産者を欠いたサイクルはノイズを信号として報告する）。予算は消費するので、後続は残りを取り合う。**部分掃引が静かであってはならない**（G-17 の形）ので 3 面に出す: `TickReport.sweep`、`InputHealth`（下記 #140）、`/healthz` と `GET /api/v3/self_eval` の新しい `sweep` モニタ（完全 = OK / 段階的 = INSUFFICIENT / **未観測の取得元を延期している = ANOMALY** / 未報告 = UNSUPPLIED、OK には決してしない）。**実測で判明した予算の限界（2026-08-09、影配備で計測。裁定を仰ぐ）**: **予算はリクエスト「数」を縛るが壁時計を縛らない。** ティック 1 は 120 リクエストを合計 66.1 秒（0.55 s/req、outcome は ok 88 / http_error 30 / connection_error 2）で終えたが、**ティック 2 は 441 秒を超えても 1 行も `fetch_log` に書かなかった**。原因はネットワークの遅さではなく `check_host` の**宣言された待ち時間**である: 同アダプタは `RequestContinuation`（`check_host.py:264-268`）を持ち、check-host.net はチェックを受け付けてハンドルだけ返し、測定は `check-result/{request_id}` に**ノードが答えた後にしか存在しない**ので `delay_sec = RESULT_DELAY_SEC = 5.0`（K03）が要る。配備実機の展開で **71 step すべてが continuation を持つ**ので、`client.pause` による**必須スリープだけで 71 × 5 = 355 秒**であり、取得を 1 バイトもしないうちに `ops_health.LOOP_MIN_OVERDUE_SEC`（300 秒）を超える。したがって **`check_host` を含むティックは構造上 `loop_overdue`（WARN → `/healthz` 503）になる**。**これを黙って直さない**理由を明記する: (a) 予算を下げても効かない — アダプタは原子的で、`check_host` 単体が 71 step である。(b) `check_host` の 1 ティックあたりの URL 数を減らすのは**測定の口径を狭める感度変更**であり、本作業の権限外である。(c) `loop_budgets` の閾値を実測の遅さに合わせて緩めるのは、**アラームを事後に静かにする**動きであり、NP1 が最も嫌う方向である（本モジュールの docstring 自身が「crying wolf」を警戒している）。**測定は正直であり、警報も正直である** — この配備は実際に 300 秒以内に結論を更新できない。裁定候補は 3 つ: (i) `check_host` の口径を per-tick で分割する（`RequestContinuation` を持つ step 群を複数ティックに分ける = 予算の単位をアダプタから step 群へ下げる）、(ii) 取得の並行化（§1-2 の裁定）、(iii) `loop_budgets` をティック実測時間から導く。**推奨は (ii)** — 355 秒はスリープであって CPU でも帯域でもないので、並行化がそのまま効く唯一の項目である。**解消条件**: 取得の並行化が合成ルートの裁定として下りたとき（§1-2 はスレッドを合成ルートの所有物と定めており、予算はその裁定を先取りしない）。証跡: `tests/test_fetch_runner.py::TestTheFetchBudgetBoundsOneTick`、`tests/test_runtime_ops_health.py::TestTheSweepMonitorSaysHowMuchOfTheCatalogueSpoke` |
| 140 | 取得カーネル全体 — **予算による延期は台帳次第で正反対の意味を持つ**（**#139 の帰結、2026-08-09**） | 採点は `now` における L1 の in-force 射影を読む（`v3/runtime/scoring.py::assemble`）ので、**直近の観測が有効期間内にあるアダプタを 1 ティック延期しても採点は前回の値から行われ、損失はゼロ**。これを失敗として数えると健全な配備で coverage ガードが発火し、正常なセンサーを故障として名指す。**冷台帳では同じ skip が正反対を意味する**: 一度も取得されていないアダプタは射影に何も持たず、寄与ゼロで、しかも `stale_sources` からも見えない（`if newest is not None` により**存在する行しか古くしない**）。区別しないままだと、**取得元の 40% だけを掃引した初回ティックが、全件掃引したティックとまったく同じ結論を出す** — G-17 そのもの。採択: `health.never_observed()`（L1 に 1 行も無いアダプタ）で解決し、`fetch_budget_deferred` を**延期かつ未観測なら `sources_failed`、延期だが有効期間内の読みがあるなら `NOT_CONSULTED`** に振り分ける | **sensitive**（冷起動中は `sensor_coverage_degraded` が発火し、静穏な結論が「結論不可 + 理由」に変わる。NP5+8 が第一級の答えとするもので、NP1 側） | **登録**（WP-D、2026-08-09）。`tl_observation` は毎ティック書かれ続ける（ガードは結論側にしか効かない）ので、**「ティックが完了して脅威レベルを書く」ことと「その結論が部分掃引だと申告する」ことは両立する**。**掘り当てた別件を申し送る（v3 側は直していない）**: `stale_sources` は `tuple(countries) or ('GLOBAL',)` と書いているため、**参加国のあるティック — つまり実運用の全ティック — では GLOBAL を一度も見ない**。結果として countryless な族（`apt_intel`、`cf_bgp_hijack`、§7-2 #127 の全行）は**構造的に stale と報告されえない**。`never_observed` は参加国**および** GLOBAL を見るので同じ穴を再生産しない。`stale_sources` 側を直すのは「使用不能と判定される取得元が増える」変更であり、独自の方向と独自の登録を要する別件として据え置く。証跡: `tests/test_runtime_health.py::TestABudgetDeferralMeansTwoDifferentThings` / `::TestNeverObservedIsAskedOfTheLedger` |
