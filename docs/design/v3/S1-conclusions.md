...
**一次ソース**: PER_DOMAIN / TREND / ATTACK_MODE の閾値は D5 の指示どおり実装値を CORE とした。ただし
**D5 §3-7 の前提「実値はテストのみが保持」は誤り**である。当該テスト群は閾値を**モジュール定数として import し、
定数相対で境界を検証する**設計で、数値そのものは pin していない。数値の唯一の所在はモジュール定数であり、
テストは「threshold_ref が定数を漏れなく公開しているか」のドリフト検知装置である（§7 GAP-01/02）。
...
- **envelope**: 結論を包む外殻（api_version / scenario_id / observed_at / disclaimer / conclusions[]）
...
**検証欄の略号**（全て `tests/` 配下）: `TL`=test_threat_level_derive / `TR`=test_trend_derive /
...
`FAL`=test_falsification / `SEV`=test_severity / `SE`=test_self_eval。「全件」= 当該ファイルの全テスト。
閾値の config キー・既定値・override 可否は §3 に一括記載する。
...
### S1-CONC-001: 5 型のみ、一意 ID、構築時に 3 不変条件を強制（違反は拒否、クランプしない）
...
以下を満たさない結論は**構築を拒否 MUST**（沈黙の補正をしてはならない）:
...
**根拠**: base.py:20-27, 43-45, 71-87、config.py:227-230
**検証**: SC::test_all_conclusion_types_construct / ::test_conclusion_id_is_unique / ::test_unavailable_conclusion_requires_null_state ほか 4 件；disclaimer は 5 型が独立検証 (TL/TR/PD/AM/AN の `test_disclaimer_*` 各 1 件)
**分類**: CORE（NP5+8 と NP7 を型で強制）

### S1-CONC-002: 結論は不変、直列化は安定、disclaimer は読み出し時に注入
**挙動**: 結論オブジェクトは**不変 MUST**（下流の加工は複製を返す **MUST**）。API 表現では列挙型を**文字列値に
落とす MUST**。永続表現では threshold_ref / source_urls / calibration_status / metadata を **JSON 文字列化 MUST**、
辞書は**キー昇順で安定化 MUST**。台帳から復元する際、disclaimer は**行の値ではなく現行 config 値を注入 MUST**
（文言改訂時に過去行が古い文言を持たない）。
**根拠**: base.py:48, 93-136、persistence.py:336-354
**検証**: SC::test_conclusion_is_frozen / ::test_to_dict_serialization_round_trip / ::test_to_db_row_serializes_json_fields ほか 1 件；AL::test_rule_conclusion_is_not_mutated（注入は未検証 — §7 GAP-06）
**分類**: CORE（NP6: 公開済み結論の事後改変を構造的に禁止）

### S1-CONC-003: 結論不可理由は 4 値の閉じた列挙 — **本書の中核 (NP5+8)**
**挙動**: 結論不可の理由は以下 4 値のみ **MUST**。理由なしの結論不可は表現できない (001)。

| 値 | 意味 | 発生条件（実装上の全経路） |
|---|---|---|
| `insufficient_data` | 判断に足るデータが無い | (a) TL が導出できない (010) / (b) TREND の 3 窓すべてが標本不足 (018) / (c) PER_DOMAIN の 3 ドメインすべてが無信号 (019) / (d) ANOMALY に採点可能な寄与が皆無 (021) / (e) ATTACK_MODE でどの規則も発火しない (026) / (f) 台帳に当該 (scenario, type) の行がまだ無い (006) |
...
**検証**: TL::test_tl_none_yields_insufficient_data；TR::test_empty_db_yields_insufficient_data；PD::test_all_domains_silent_yields_insufficient_data；AN::test_no_contributions_yields_single_insufficient_data；AM::test_no_signals_yields_insufficient_data / ::test_below_all_floors_yields_insufficient_data；API::test_bundle_endpoint_returns_unavailable_for_empty_scenario
**分類**: CORE。**4 値中 3 値が未使用である点は ACCIDENTAL**（§5-A1）

### S1-CONC-004: 結論不可は「否定の主張」ではない
**挙動**: 結論不可の結論は、**当該事象が存在しないことを主張してはならない MUST**。とくに ATTACK_MODE の
結論不可は「攻撃なし」ではなく「規則が判断を放棄した」を意味する。結論不可行には `metadata.is_transient`
（真偽）と `metadata.reason_detail`（平文の理由）を**必ず添える MUST**。
**根拠**: attack_mode.py:96-113、threat_level.py:127-135、trend.py:122-126、per_domain.py:107-112、anomaly.py:129-134
**検証**: AM::test_no_signals_yields_insufficient_data / ::test_insufficient_metadata_carries_domain_scores；TR::test_empty_db_yields_insufficient_data；AN::test_no_contributions_yields_single_insufficient_data
**分類**: CORE（NP1: 見逃しを「無害」と誤読させない）

### 2.2 v2 API envelope

### S1-CONC-005: envelope は固定 5 フィールド、disclaimer は二重掲示、エラーも例外でない
...
`observed_at` 未指定時は**同梱結論の観測時刻の最大値 MUST**（空なら現在時刻）。**4xx / 5xx を含むすべての
v2 応答**が api_version と disclaimer を持つ **MUST**。v2 面全体を単一フラグで無効化でき、無効時は全
エンドポイントが **503 + NP7 準拠 body MUST**。
**根拠**: api.py:20-56、routes/conclusions_v2.py:56-66、config.py:244
**検証**: API::test_bundle_endpoint_returns_latest_of_each_type / ::test_v2_error_responses_carry_disclaimer / ::test_v2_returns_503_when_flag_off ほか 1 件；FB::test_post_503_when_v2_disabled / ::test_get_503_when_v2_disabled；FB2::TestEndpoint::test_returns_v2_envelope（observed_at 既定は未検証 — §7 GAP-06）
**分類**: CORE（NP7 の抜け穴封じ）。**フラグが恒常 true の移行足場である点は D2 C-03**（v3 で撤去 MUST）

### S1-CONC-006: 台帳行が無い (scenario, type) は「まだ言うことがない」を 200 で返す
**挙動**: 台帳に行が無い場合、404 ではなく**結論不可 envelope を 200 で返す MUST**。理由は `insufficient_data`、
`confidence=0.0`、`formula_ref=null`、`threshold_ref={}`、`source_urls=[]`、`metadata.is_transient=true`。
...
### S1-CONC-007: bundle は 5 型それぞれの最新行、不正入力は 400/404
**挙動**: シナリオ単位の bundle は 5 型それぞれについて**独立に最新 1 行**を集めて返す **MUST**（1 型が古くても
他型は最新を返す）。1 型も無ければ 006。未知の conclusion_type は **400 MUST**（有効値一覧を detail に）、
未知の結論 ID は **404 MUST**。
**根拠**: routes/conclusions_v2.py:134-143, 153-166, 176-182
**検証**: API::test_bundle_endpoint_returns_latest_of_each_type / ::test_bundle_endpoint_picks_newest_per_type / ::test_single_type_endpoint_returns_only_that_type ほか 3 件
**分類**: CORE

### S1-CONC-008: 監査トレースは導出鎖を全部開示する
...
**検証**: API::test_audit_trace_returns_full_disclosure_without_llm / ::test_audit_trace_resolves_llm_prompt_when_sha_set / ::test_audit_trace_marks_llm_prompt_missing_when_row_purged ほか 1 件
**分類**: CORE（NP6 の到達点）

### 2.3 THREAT_LEVEL 結論

### S1-CONC-009: state は TL 整数の文字列、比較は必ず severity 経由
...
**検証**: TL::test_state_is_string_of_tl_int；SEV::test_tl_constants_are_defcon_style / ::test_severity_of_is_monotonically_decreasing_in_tl / ::test_severity_of_rejects_out_of_range_tl ほか 2 件
**分類**: CORE（較正災害 2 件の再発防止番兵）

### S1-CONC-010: TL が出せないときも行を出す
**挙動**: スコアリングが TL を決められなかった場合、行を**黙って捨ててはならない MUST**。`insufficient_data` の
結論を発行し、`metadata.reason_detail` に導出関数が None を返した旨を記す **MUST**（慢性結論不可検知 §2.9 の
観測列を途切れさせない）。
...
### S1-CONC-011: TL の confidence はスコアの線形写像をクランプしたもの、lite は割引
...
**検証**: TL::test_confidence_clamped_to_unit_interval；test_scenario_scoring.py::TestLiteTlConfidenceDiscount::（2 件、S1-SCORE-017 と共有）
**分類**: CORE（NP4 で結論は出す、NP5+8 で品質差を可視化）。**分母 12.0 は暫定値 = ACCIDENTAL**（§5-A3）

### S1-CONC-012: TL 結論は使用した閾値表を自ら公開する
**挙動**: `threshold_ref` として `tl1_total`(9.0) / `tl1_physical`(3.0) / `tl2_total`(6.0) /
`tl2_active_domains_min`(2) / `tl3_total`(4.0) / `tl4_total`(2.0) の 6 キーを**公開 MUST**。値は
...
### S1-CONC-013: 導出根拠マトリクスと一次ソースを添える
...
**検証**: TL::test_metadata_includes_rationale_matrix_when_contributions_exist / ::test_rationale_matrix_is_empty_list_when_no_contributions / ::test_rationale_matrix_sorted_by_absolute_contribution ほか 4 件
**分類**: CORE（NP6 の最小単位。S1-SCORE-009 の formula_trace をそのまま搬送する）

### S1-CONC-014: 反証レポート — 「何が変われば結論が変わるか」を決定論的に添える
...
3. **signal_sensitivity**: 寄与上位 3 件について、その信号がゼロに落ちた場合の仮想 TL と移動段数を算出 **MUST**。
...
### S1-CONC-015: TREND は 3 窓を 1 行に詰め、一次ソースを持たない
**挙動**: `short_term`(24h) / `medium_term`(7d) / `long_term`(30d) の 3 窓を評価し、**1 個の結論行**に packed state
`short_term=X;medium_term=Y;long_term=Z` として格納 **MUST**（順序固定）。同内容を `metadata.windows` にも持つ。
TREND は台帳の TL 行から算出されるため `source_urls` は**常に空 MUST**（生センサー URL を偽って添えない）。
**根拠**: trend.py:50-54, 140, 208-217
**検証**: TR::test_state_string_packs_all_three_windows_in_canonical_order / ::test_state_string_uses_module_window_order / ::test_returns_single_conclusion_of_trend_type ほか 4 件
**分類**: CORE（NP6: 出典を捏造しない）

### S1-CONC-016: 窓の評価は「現在期間の平均 severity − 直前期間の平均 severity」
...
**検証**: TR::test_stable_when_delta_within_rising_band / ::test_rising_when_delta_above_rising_below_escalate / ::test_escalating_when_delta_at_or_above_escalate_delta ほか 3 件（**定数相対** — §7 GAP-02）
**分類**: CORE。**設計文書 §6.2 の目標語彙 (RAPIDLY_* 系) との乖離は ACCIDENTAL**（§5-A5）

### S1-CONC-018: 標本数は両期間 3 件以上、1 窓でも判定できれば結論は available
...
**1 窓でも判定できたら結論は available MUST**、**3 窓すべて判定不能のときのみ `insufficient_data` MUST**。
confidence は各窓で `n = min(current_n, previous_n) >= 3` なら `min(0.30, 0.15 + n × 0.01)` を加算し、合計を
**0.95 で上限クリップ MUST**。判定不能の窓が 1 つでもあれば **0.15 減点 MUST**（下限 0.0、小数 3 桁）。
結論不可時は **0.0 MUST**。
**根拠**: trend.py:71, 99-127, 175-180, 220-234
**検証**: TR::test_below_min_samples_in_one_window_marks_that_window_unavailable / ::test_insufficient_data_reports_zero_sample_counts / ::test_partial_availability_penalises_confidence ほか 3 件
**分類**: CORE（NP4 結論最大化）

### 2.5 PER_DOMAIN 結論

### S1-CONC-019: 3 ドメインを 1 行に詰め、1 ドメインでも信号があれば結論を出す
...
3 ドメインすべてが INSUFFICIENT_SIGNAL のときのみ `insufficient_data` **MUST**（NP1）。
`confidence = 0.5 × (信号のあるドメイン数 / 3) + 0.5 × min(1.0, ドメインスコア合計 / 12.0)`、小数 3 桁 **MUST**。
**根拠**: per_domain.py:41, 71-115, 128-132, 148-149, 165-173
**検証**: PD::test_state_string_packs_all_three_domains_in_canonical_order / ::test_source_urls_only_count_known_domains / ::test_all_domains_silent_yields_insufficient_data ほか 8 件
**分類**: CORE

### S1-CONC-020: ドメイン状態の分類は 5 値・順序依存
...
**根拠**: per_domain.py:51-53, 136-145（2026-04-26 Phase 1.3 の実分布較正値）
**検証**: PD::test_active_state_at_or_above_active_floor / ::test_active_floor_boundary_inclusive / ::test_degrading_when_drop_at_or_above_degrade_delta ほか 9 件（**すべて定数相対** — §7 GAP-02）
**分類**: CORE。**DEGRADING が ELEVATED より優先される順序は ACCIDENTAL**（§5-A6）

### 2.6 ANOMALY 結論

### S1-CONC-021: 1 ティックに複数行、上位 N 件、state は統制語彙、皆無なら単一の結論不可行
...
返す MUST**（空リストを返してはならない — 慢性検知の観測列が切れる）。`state` は **signal_source のみ MUST**
（自由記述の要約文字列を入れてはならない — 行ごとに一意な state になり同種異常の集計が不可能になる）。
自由記述は `metadata.value_display` に置く **MUST**。各行の `source_urls` は**その信号自身の evidence_url のみ
MUST**、各行に calibration_status を**添える MUST**。
**根拠**: anomaly.py:64, 96-112, 186-187, 218-246
**検証**: AN::test_returns_list_of_anomaly_conclusions / ::test_top_n_limit_truncates / ::test_state_summary_uses_signal_source_with_value_in_metadata ほか 7 件
**分類**: CORE（NP5+8 + NP6 集計可能性）。**設計文書 §6.4 は旧 state 仕様のまま**（§5-A9）

### S1-CONC-022: 重要度は 4 因子の積 × 100、全因子を開示する
...
**検証**: AN::test_metadata_captures_every_formula_component / ::test_importance_clamped_to_unit_confidence / ::test_recency_decay_at_one_time_constant ほか 7 件
**分類**: CORE。**設計文書が 12h を "half-life" と誤記していた点は ACCIDENTAL**（§5-A7）

### S1-CONC-023: 新規性は同一 signal_source の直近 24h 出現数で減衰し、読み失敗は必ず標識を残す
...
**検証**: AN::test_novelty_factor_drops_for_repeated_signal_source / ::test_novelty_isolated_per_scenario / ::test_novelty_falls_back_when_ledger_query_fails ほか 4 件
**分類**: CORE。**change gate との相互作用（台帳行減 → novelty 上振れ）は ACCIDENTAL**（§5-A8）

### 2.7 ATTACK_MODE 結論

### S1-CONC-024: 複数同時発火、最上位を state に、全件を metadata に、低確信には旗を立てる
...
**検証**: AM::test_multi_mode_records_top_in_state_and_full_ranked_list / ::test_each_ranked_entry_carries_rule_string / ::test_tentative_flag_set_when_confidence_low ほか 5 件
**分類**: CORE

### S1-CONC-025: 4 つの基本モードの発火規則
...
**根拠**: attack_mode.py:50-56, 140-184（2026-05-10 の実分布再較正値）
**検証**: AM::test_ddos_precursor_fires_at_floors / ::test_kinetic_preparation_fires_at_floor / ::test_hybrid_pressure_requires_all_three_domains_and_cluster ほか 9 件（**すべて定数相対。数値そのものは未 pin** — §7 GAP-01）
**分類**: CORE。**設計文書 §6.5 が再較正前の値のままである点は ACCIDENTAL**（§5-A10）

### S1-CONC-026: どの規則も発火しなければ結論不可 — 「攻撃なし」ではない
...
### S1-CONC-027: シナリオ固有拡張モードは宣言的設定のみで定義し、不正宣言は黙って捨てる
...
降順で再ソート）。基本も拡張も発火しなければ**結論不可のまま MUST**（拡張で結論不可を無理に結論化しない）。
confidence は `clamp(base_confidence, 0.55, 0.85) + 0.30 × min(1, max(margin))` を**0.95 で上限クリップ MUST**
（margin = 各 domain_floor に対する相対超過、floor 0 は除外）。以下は**黙って捨てる MUST**（設定不備でツールを
止めない — NP3）: mode が非文字列 / 空 / 基本 5 モード名と衝突（警告ログのみ）、辞書でないエントリ、同一 mode の
重複宣言（**最初の宣言が勝つ MUST**）、拡張ブロックが配列でない、設定が読めない。
**根拠**: attack_mode_extensions.py:62-64, 106-227、attack_mode.py:71-77, 93-114
**検証**: AX::全件（26。実設定 5 シナリオの live parametrize 5 件を含む）
**分類**: CORE（ADR-V2-002）

### S1-CONC-028: LLM 補強は規則結論を上書きしない
...
### S1-CONC-029: 全結論に較正スナップショットを添え、出典は ground-truth 混同行列に限る
...
恒等的に 0 となり、360k 件の結論すべてで「異常なし」を報告した実績がある）。取得は**読み取り専用 MUST**、失敗時は
**INSUFFICIENT_DATA の封筒を返し例外を伝播させない MUST**（NP3）。シナリオ単位で **300 秒メモ化 MUST**、
新ラベル投入後は明示的に無効化できる **MUST**。
**根拠**: calibration.py:27-51, 76, 98-191
**検証**: TL::test_calibration_status_is_attached；AN::test_calibration_status_attached_to_each_row；CAL::test_never_raises_on_bad_db / ::test_cache_returns_same_until_invalidated
**分類**: CORE（NP5+8 + NP3）

### S1-CONC-030: 較正状態は 3 値、判定は recall の分母で決まる
...
**検証**: CAL::test_insufficient_data_when_no_labels / ::test_ok_when_recall_holds / ::test_degraded_when_recall_below_floor ほか 3 件（dedup 規則は未検証 — §7 GAP-06）
**分類**: CORE

### 2.9 台帳への書込と結論不可の継続性

### S1-CONC-031: 台帳は append-only、単一値型の書込は「変化 + heartbeat」でゲートする
**挙動**: 結論台帳は追記のみ **MUST**（既存行の UPDATE / DELETE を行ってはならない **MUST**。retention による
一括削除を除く）。訂正は新しい ID の行を追記する。単一値型（threat_level / trend / per_domain / attack_mode）は
**同一 (scenario, type) の最新行と比較して** `state` と `conclusion_unavailable_reason` がともに等しく、かつ経過
時間が heartbeat 周期未満なら**書込を省略 MUST**。比較対象は**型の最新行であって「同じ state の最新行」ではない
MUST**（A→B→A のフラップで 3 行書かれないと、任意時点の再構成が B のまま固まる）。ゲート無効時は毎ティック書く **MUST**。
**根拠**: persistence.py:1-11, 29-55, 84-110、config.py:264-269
**検証**: PER::（round-trip 群）；GATE::TestSingleValuedGate::test_unchanged_state_is_deduplicated / ::test_flap_writes_every_transition / ::test_heartbeat_forces_periodic_write ほか 2 件
**分類**: CORE（ADR-V2-008 / AP4 の前提）

### S1-CONC-032: 複数行型は「集合シグネチャ」でゲートする
...
**検証**: GATE::TestBatchGate::（3 件）/ ::TestBatchGateTimestampJitter::（1 件）/ ::TestBatchGateIdentitySet::（2 件）
**分類**: CORE

### S1-CONC-033: 継続性台帳は毎ティック更新し、遷移だけを記録する
**挙動**: 書込が間引かれたティックでも、**結論不可の継続性記録は必ず行う MUST**（慢性検知には間引き前の観測列が要る）。
(scenario, type) ごとに **MUST**: 結論不可 ∧ 直近行が無い/解消済 → **新しい run を開く**（run_length=0、
first_seen=現時刻）/ 結論不可 ∧ 直近行が未解消 → **run を延長**（run_length = 現時刻 − first_seen）/
...
**根拠**: persistence.py:78-81, 106-108, 198-201, 207-293
**検証**: GATE::TestSingleValuedGate::test_deduplicated_tick_still_feeds_continuity；PER::TestContinuityLog::test_unavailable_conclusion_opens_run / ::test_consecutive_unavailable_extends_run_length / ::test_available_after_unavailable_closes_run ほか 1 件
**分類**: CORE（NP5+8 と AP4 の両立点）。**state 文字列による二重判定は ACCIDENTAL**（§5-A11）

### S1-CONC-034: 慢性結論不可は 2 基準の論理和で判定する — NP5+8 の設計失敗検知
...
run の合計時間**を積算し、窓境界で**クランプ MUST**、1.0 で上限クリップ **MUST**。連続日数閾値は**下限 3.0 日を
下回れない MUST**。
...
### S1-CONC-035: 慢性スナップショットは 2 基準を重複なくマージし、個別行は滞留を自己申告する
...
**検証**: IC::TestChronicSnapshot::（2 件）/ ::TestChronicSnapshotDutyMerge::（2 件）（null_run_minutes 付与は未検証 — §7 GAP-05）
**分類**: CORE（AP3）

### S1-CONC-036: 台帳は保持期間で刈られ、プロンプトは結論より長く残る
...
### S1-CONC-037: replay は台帳の時間旅行であり、物語の再生成ではない
...
再水和 MUST**。応答は `api_version` / `scenario_id` / `replay_at` / `conclusions` / disclaimer。
**アナリスト権限を要求 MUST**。
**根拠**: routes/conclusions_v2.py:468-541
**検証**: GATE 群が「latest-row-at-T の意味論」を間接保証（031 / 032）。**endpoint 自体は未検証**（§7 GAP-04）
**分類**: CORE（AP4）

### S1-CONC-038: self_eval は信頼メトリクスを独立に算出し、決して失敗しない
...
| メトリクス | 定義 |
|---|---|
| `recall` | **CI ゲートと同一の集計経路 MUST**（チップとゲートが別の数字を出すことを構造的に禁止）。窓は per-conclusion 較正 (030) と**同一日数 MUST**。`ΣTP / (ΣTP + ΣFN)`、分母 0 なら null。`recall_meta` に**窓日数・総/人手/自動ラベル数を開示 MUST**（人手 0 = 完全な自己採点を検知できること） |
...
**検証**: SE::test_self_eval_drift_null_when_no_calibration_data / ::test_self_eval_drift_mean_miss_rate / ::test_self_eval_recall_uses_calibration_window_and_reports_breakdown ほか 2 件（null_zone / skew は未検証 — §7 GAP-06）
**分類**: CORE（AP3 + NP3 + D2 D-01 への構造的対抗）

### 2.11 Markdown エクスポート

### S1-CONC-039: エクスポートは 1 シナリオの最新結論を、導出鎖ごと 1 ファイルにまとめる
**挙動**: 5 型それぞれの最新行を集め、**単一の Markdown 文書**として返す **MUST**。文書レベルで**必ず含む MUST**:
シナリオ名（無ければ ID）/ シナリオ ID / 生成時刻（UTC, ISO8601 秒精度）/ API バージョン /
**NP7 disclaimer（先頭に 1 回のみ、引用ブロック）**。結論が 1 件も無ければ**プレースホルダ文を出す MUST**
（空文書にしない）。出力は**末尾改行 1 個で終わる MUST**。節レベルで**必ず含む MUST**: 型見出し / **状態**
（結論不可なら「結論不可」と理由コード）/ 確信度（小数 2 桁）/ 観測時刻 / 結論 ID / 導出式参照 / **閾値（JSON）**。
source_urls / calibration_status / metadata は**存在するときのみ節を出す MUST**（空節を作らない）。JSON は
**キー昇順で整形 MUST**。監査トレースが渡され、かつプロンプトが欠落でない場合は **LLM プロンプト全文を折りたたみ節で
埋め込む MUST**（sha256 / model / temperature 付き）。トレースが無い、または欠落マーク付きなら**出さない MUST**。
**根拠**: markdown.py:54-165
**検証**: MD::全件（21）；API::test_md_export_returns_markdown_content_type / ::test_md_export_includes_audit_trace_when_requested ほか 3 件
**分類**: CORE（NP6 の可搬形）

### 2.12 アナリスト feedback 台帳

### S1-CONC-040: ラベルは 4 値の混同行列で追記のみ、analyst_id と時刻はサーバが決める
...
**検証**: FB::test_db_check_constraint_rejects_unknown_label / ::test_post_creates_row_with_server_derived_analyst_id / ::test_post_truncates_overlong_notes ほか 4 件
**分類**: CORE（ADR-V2-011）

### S1-CONC-041: 集計は「単一の評決」ではなく「誰が何と言ったか」を返す
**挙動**: 結論単位の集計は **4 ラベルすべてのカウント（0 を含む）と延べ数と相異なるアナリスト数**を返す **MUST**。
単一の合成評決を返してはならない **MUST**（アナリストバイアス緩和）。ラベルが 1 件も無い結論でも **404 ではなく
ゼロ集計を返す MUST**。一覧は新しい順、既定 50 件・上限 1000 件 **MUST**。結論横断の集計は**アナリスト種別
（人手 / 自動）で絞り込める MUST**（自動ラベルは analyst_id が `auto:` 接頭辞を持つもの **MUST**）。返す内容は
窓時間 / 総数 / 人手数 / 自動数 / 相異なるアナリスト数 / ラベル別内訳 / **結論型別内訳** / recall / precision
（分母 0 なら null）**MUST**。
**根拠**: feedback.py:84-130, 165-300、routes/conclusions_v2.py:387-425
**検証**: FB::test_summarize_returns_zero_counts_for_empty_conclusion / ::test_get_empty_returns_zero_summary_not_404 ほか 3 件；FB2::全件（13）
**分類**: CORE（AP3: recall が自己採点だけになっていないかを見える化する）

### S1-CONC-042: 人手が確認した実事象は独立の ground-truth 台帳にも記録する
...
### S1-CONC-043: 人手ラベルの独立性が calibration の唯一の構造的担保である
...
**検証**: HA::TestSelection::test_auto_fn_review_candidate_surfaces / ::test_human_answered_conclusion_leaves_queue / ::test_peak_severity_picks_most_severe_call ほか 2 件；::TestQueueEndpoint::（2 件）
**分類**: CORE（D2 D-01「測定系のバグ」への唯一の構造的対抗策）

### S1-CONC-044: アナリストには自然言語の 1 問だけを問い、ラベルはツールが導出する
...
**検証**: HA::TestSelection::test_answer_model_alert_stance_maps_yes_to_true_positive / ::test_answer_model_calm_stance_maps_yes_to_false_negative / ::test_answer_model_question_names_the_scenario ほか 1 件
**分類**: CORE（AP2）

### S1-CONC-045: 外部調査リンクは調査を自動化するが判断は自動化しない
...
|---|---|---|---|---|
| NP7 disclaimer 文言 | "Tool conclusion only — final judgment by organizational process." | `V2_NP7_DISCLAIMER` | 不可 | 001 |
| v2 API 有効 / 台帳書込有効 | True / False | `V2_API_ENABLED` / `V2_CONCLUSION_LEDGER_ENABLED` | 不可 | 005, 031 |
| TL confidence 分母 / lite 係数 | 12.0 / 0.6 | — | 不可 | 011 |
| TL 閾値表（公開値） | 9.0 / 3.0 / 6.0 / 2 / 4.0 / 2.0 | — | 不可 | 012（原典 S1-SCORE-004） |
| falsification 上位 N / domain_cap | 3 / 6.0 | — | 不可 | 014 |
| TREND 窓 | 24h / 7d / 30d | — | 不可 | 015 |
| TREND RISING / ESCALATE_DELTA | **0.50 / 1.50** | — | 不可 | 017 |
| TREND MIN_SAMPLES / confidence 上限・減点 | 3 / 0.95・0.15 | — | 不可 | 018 |
| PER_DOMAIN ACTIVE / ELEVATED / DEGRADE | **2.5 / 1.5 / 1.0** | — | 不可 | 020 |
| PER_DOMAIN magnitude 分母 | 12.0 | — | 不可 | 019 |
| ANOMALY 既定件数 / 重要度上限 / 経年時定数 | 10 / 100.0 / 12.0h | — （`?limit=`） | 不可 | 021, 022 |
| ANOMALY novelty 母数 / 下限 / 窓 | 10 / 0.3 / 86400s | — | 不可 | 023 |
| ATTACK_MODE cyber / info / physical / hybrid floor | **1.2 / 0.8 / 1.0 / 0.8** | — | 不可 | 025 |
...
| null_severity 段階 | 7 日 / 24 時間 | — | 不可 | 035 |
| 結論 / プロンプト台帳 保持 | 90 日 / 120 日（下限 = 結論 +30） | `CONCLUSIONS_RETENTION_DAYS` / `LLM_PROMPTS_RETENTION_DAYS` | 不可 | 036 |
| null-zone 走査 / TL5 偏り警報 | 30 日 / `CALIBRATION_SKEW_*` 4 キー | `CALIBRATION_SKEW_METRIC_ENABLED` ほか | 不可 | 038 |
| feedback 既定窓 / 行上限 | 720h（最大 1 年） / 2000 | — | 不可 | 041 |
| 人間アンカー キュー長 / 窓 | 5 / 7 日（endpoint で窓 1-30 日・件数 1-20 に制限） | `HUMAN_ANCHOR_QUEUE_SIZE` | 不可 | 043 |
| ground-truth 前方窓 / FP 地平 / alert 最小 severity | 72h / 7 日 / 3 | `GROUND_TRUTH_WINDOW_HOURS` / `GROUND_TRUTH_FALSE_POSITIVE_HORIZON_DAYS` | 不可 | 043, 044 |
...
## 4. 文書との乖離（実装/テストを CORE とし、文書側の訂正を要する。個別に数えて 14 件）
...
| 9-11 | v2-migration §6.2/§6.3/§6.5: velocity+acceleration ベース、ACTIVE は raw>5.0 ∧ signal_count≥5 + 30% 相対 DEGRADING、DDOS は cyber_signal_count≥5 ∧ narrative_burst≥3 | いずれも「現ティックのドメインスコアの絶対閾値」に置換済（3 件） | 文書側 drift 記載済 |
| 12 | v2-migration §6.4: ANOMALY state は要約文字列 | **signal_source の統制語彙**（Phase G 2026-04-30） | §5-A9 |
...
|---|---|---|---|
| test_anomaly_derive | 29 | BEHAVIOR | 001, 003, 004, 021, 022, 023, 029 |
| test_per_domain_derive | 27 | BEHAVIOR | 001, 003, 019, 020 |
| test_attack_mode_derive | 26 | BEHAVIOR | 001, 003, 004, 024, 025, 026 |
| test_attack_mode_extensions | 26 | BEHAVIOR | 027 |
| test_trend_derive | 23 | BEHAVIOR | 001, 003, 015〜018 |
| test_conclusions_markdown | 21 | CONTRACT | 039 |
| test_conclusions_api | 19 | CONTRACT | 003, 005〜008, 039 |
| test_human_anchor | 17 | BEHAVIOR | 042, 043, 044, 045 |
| test_inconclusive_continuity | 17 | BEHAVIOR（endpoint 部は CONTRACT） | 034, 035 |
| test_threat_level_derive | 15 | BEHAVIOR | 001, 003, 009〜013, 029 |
| test_conclusions | 14 | CONTRACT | 001, 002 |
| test_conclusions_feedback | 14 | CONTRACT | 005, 040, 041 |
| test_attack_mode_llm_augment | 14 | BEHAVIOR | 002, 028 |
| test_conclusions_persistence | 14 | STRUCTURAL | 031, 033 |
| test_analyst_feedback_v2 | 13 | CONTRACT | 005, 041 |
| test_conclusion_write_gating | 12 | BEHAVIOR | 031, 032, 033, 037 |
| test_falsification | 12 | BEHAVIOR | 014 |
| test_conclusion_calibration | 8 | BEHAVIOR | 029, 030 |
| test_severity | 8 | BEHAVIOR | 009（うち 3 件は ground-truth ラダー = **S1-calibration 担当**） |
| test_self_eval | 9 | CONTRACT | 038 |
...
| GAP-05 | 結論不可行の `null_run_minutes` / `null_severity` 付与（035）が未検証 | 低〜中 |
| GAP-06 | 002（disclaimer の読み出し時注入）/ 005（envelope observed_at 既定）/ 030（(結論, アナリスト) 最新 1 件 dedup）/ 038 の null_zone・skew が未検証 | 中。030 は recall の分母に、038 は「TL5 が 0% に落ちる」型の較正劣化検知に直結 |
...
4. **人間アンカーの実効性**: 週次キューは実装済だが、実際に人手ラベルが供給されているか（`recall_meta.labels_human`
   の実測）は D4 の範囲。**人手ラベル 0 が続く場合、S1-CONC-043 の存在意義が形骸化する**ため、v3 では供給量そのものを
   AP3 の監視対象にすべき
