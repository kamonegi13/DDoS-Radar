...
- Recall metrics CI ゲート — **実装済 (2026-04-27)**: `scripts/check_recall_baseline.py` + `test_check_recall_baseline.py` (13 件)。`docs/baselines/recall_metrics.json` をベースラインとし、per-cell recall 低下 > `--max-drop` (既定 0.05) で hard fail、coverage loss (baseline 数値→current None) も hard fail、新 cell / 軽微な slip は info ログ。bootstrap mode (baseline 不在) は exit 0 + warning で先に CI へ配線可能。`scripts/check_ci.sh` の 4 番目のゲートとして組込み。analyst_feedback 蓄積後に `--update` で初期 snapshot を commit、Design W opt-in と同時に `opt_in: true` へ flip して厳格化
