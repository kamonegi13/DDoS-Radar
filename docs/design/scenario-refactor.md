# シナリオ中心リファクタリング 設計ドキュメント (v1 凍結)

> **⚠️ ステータス: v1 系列凍結 — 新規変更は v2-migration.md へ**
>
> 本書は v1.x リファクタリング (国単位 → シナリオ単位) の凍結資料。
> CLAUDE.md で再定義された新ツール目的 (NP1-NP7) への整合は **v2.0 で対応**。
>
> **新セッションでの設計判断は [docs/design/v2-migration.md](v2-migration.md) を優越参照とする**。
> 本書は v1 系列の履歴保全と Phase 1-5 で確立した設計判断 (ADR-001〜026) の参照源として残す。
>
> v2.0 への移行判断 (旧 P5 廃止、Conclusion Model 導入、theater 撲滅、API v2、Analyst Workbench 等) は
> すべて v2-migration.md §4 (ADR-V2-001〜012) に記載。
...
| **作成日** | 2026-04-11 |
| **凍結日** | 2026-04-25 |
| **凍結理由** | CLAUDE.md NP1-NP7 再定義に伴い v2.0 移行設計を別書 (v2-migration.md) に分離 |
| **現在のフェーズ** | **v1 系列完了 / v2.0 へ移行中 (v2-migration.md 参照)** |
