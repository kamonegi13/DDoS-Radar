...
Covers:
  ...
  * /api/v2/decisions/tl_recalibration/{accept,extend,raise} require
    admin and (for raise) np7_confirmed + reason.
  * /api/v2/decisions/dual_weight/{accept,extend,rollback} require
    admin and (for rollback) np7_confirmed + reason.
...
def test_api_tl_recal_accept_admin_scope(client, admin_headers):
    resp = client.post("/api/v2/decisions/tl_recalibration/accept",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"scenario_id": "test_taiwan"})
    assert resp.status_code == 200
    body = resp.get_json()
    assert body["scenario_id"] == "test_taiwan"
    assert body["extension_days"] == 30


def test_api_tl_recal_accept_requires_scenario_id(client, admin_headers):
    resp = client.post("/api/v2/decisions/tl_recalibration/accept",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={})
    assert resp.status_code == 400
...
def test_api_tl_recal_extend_clamps_days(client, admin_headers):
    resp = client.post("/api/v2/decisions/tl_recalibration/extend",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"scenario_id": "test_taiwan", "days": 999})
    assert resp.status_code == 200
    assert resp.get_json()["days"] == 90


def test_api_tl_recal_raise_requires_np7_confirm(client, admin_headers):
    """Recall-reducing action must reject without np7_confirmed."""
    resp = client.post("/api/v2/decisions/tl_recalibration/raise",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"scenario_id": "test_taiwan",
                              "new_thresholds": {"total": 9.5},
                              "reason": "test"})
    assert resp.status_code == 400
    assert resp.get_json()["error"] == "np7_confirmation_required"


def test_api_tl_recal_raise_requires_reason(client, admin_headers):
    resp = client.post("/api/v2/decisions/tl_recalibration/raise",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"scenario_id": "test_taiwan",
                              "new_thresholds": {"total": 9.5},
...
def test_api_tl_recal_raise_succeeds_with_full_confirm(client, admin_headers):
    resp = client.post("/api/v2/decisions/tl_recalibration/raise",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"scenario_id": "test_taiwan",
                              "new_thresholds": {"total": 9.5},
                              "reason": "test calibration adjust",
                              "np7_confirmed": True})
    assert resp.status_code == 200, resp.get_json()
    assert resp.get_json()["new_thresholds"]["total"] == 9.5


def test_api_dual_weight_rollback_requires_np7(client, admin_headers):
    resp = client.post("/api/v2/decisions/dual_weight/rollback",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"reason": "test"})
    assert resp.status_code == 400
    assert resp.get_json()["error"] == "np7_confirmation_required"
...
def test_api_dual_weight_rollback_succeeds_with_confirm(client, admin_headers):
    resp = client.post("/api/v2/decisions/dual_weight/rollback",
                        headers={**admin_headers, "Content-Type": "application/json"},
                        json={"reason": "test rollback", "np7_confirmed": True})
    assert resp.status_code == 200
    assert resp.get_json()["decision_id"].startswith("dec_")
